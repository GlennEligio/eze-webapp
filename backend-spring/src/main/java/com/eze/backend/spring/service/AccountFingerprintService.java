...
//
//    @Override
//    public AccountFingerprint get(Serializable accountUsername) {
//        return repository.findByAccountUsername(accountUsername.toString()).orElseThrow(() -> new ApiException(notFound(accountUsername), HttpStatus.NOT_FOUND));
//    }
//
//    @Override
//    public AccountFingerprint create(AccountFingerprint acFingerprint) {
//        if (acFingerprint.getAccount() != null && acFingerprint.getAccount().getUsername() != null) {
//            // Check if account with accountCode exist
//            Account account = accountService.get(acFingerprint.getAccount().getUsername());
